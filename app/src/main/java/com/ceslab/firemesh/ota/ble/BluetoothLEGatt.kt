...
import com.ceslab.firemesh.myapp.MyBuildConfig
import java.util.*
...
open class BluetoothLEGatt internal constructor(private val context: Context, protected val device: BluetoothDevice) {
        ...
        synchronized(this) {
            ...
            if (hasGattServices) {
                ...
                for (gattService in gattServices!!) {
                    Log.e("displayGattServices()", "Device has service: " + getServiceName(
                        gattService.uuid
                    )
                    )
    ...
    companion object {
        ...
        private fun getServiceName(uuid: UUID): String {
            val service = GATT_SERVICE_DESCS[getIdentification(
                uuid
            )]
        ...
        private fun getCharacteristicName(uuid: UUID): String {
            val characteristic = GATT_CHARACTER_DESCS[getIdentification(
                uuid
            )]
    ...
    init {
        gattCallback = object : BluetoothGattCallback() {
            ...
            override fun onCharacteristicRead(gatt: BluetoothGatt, characteristic: BluetoothGattCharacteristic, status: Int) {
                if (status == BluetoothGatt.GATT_SUCCESS) {
                    Log.i("onCharacteristicRead", "Char " + getCharacteristicName(
                        characteristic.uuid
                    ) + " <- " + getCharacteristicsValue(
                        characteristic
                    )
                    )
                    characteristics.onRead(
                        getIdentification(
                            characteristic.uuid
                        ),
                        getCharacteristicsValue(
            ...
            override fun onCharacteristicWrite(gatt: BluetoothGatt, characteristic: BluetoothGattCharacteristic, status: Int) {
                if (status == BluetoothGatt.GATT_SUCCESS) {
                    Log.d("onCharacteristcWrite", "Char " + getCharacteristicsValue(
                        characteristic
                    ) + " -> " + getCharacteristicName(
                        characteristic.uuid
                    )
